  ...
  .group("/api/data", (app) =>
    app
      ...
      .post(
        "/preview",
        async ({ body, user }) => {
          if (!user || user.role !== "ADMIN") return { error: "Unauthorized. Admin required." };
          ...
          if (file) {
             ...
             try {
               ...
               if (!res.ok) return { error: "Failed to download Google Sheet. Ensure it is visible to Anyone with the link." };
          ...
          const normalized = parsedData.map(row => {
            const rawGuid = row.guid || row.GUID || row.id || row["GUID клиента"] || "csv";
            return {
            guid: `${rawGuid}-${uuidv4().slice(0, 8)}`,
            gender: row.gender || row.Gender || row.Пол || row["Пол клиента"] || null,
            birthDate: row.birthDate || row.birth_date || row["Дата рождения"] || null,
            segment: row.segment || row.Segment || row.Сегмент || row["Сегмент клиента"] || null,
            description: row.description || row.Description || row.Описание || row["Текст обращения"] || row["Описание "] || "",
            country: row.country || row.Country || row.Страна || null,
            city: row.city || row.City || row.Город || row["Область"] || row["Населённый пункт"] || null,
          ...
          return { success: true, previewId, data: normalized.slice(0, 50), totalRecords: normalized.length };
        }
      )
      .post(
        "/confirm",
        async ({ body, user }) => {
          ...
          try {
             ...
             for (let i = 0; i < toInsert.length; i += chunkSize) {
               ...
               console.log("Attempting to insert chunk starting with:", chunk[0]);
               try {
                 const insertRes = await db.insert(tickets).values(chunk).onConflictDoNothing({ target: tickets.guid });
